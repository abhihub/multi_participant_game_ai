...
export default function ModeratorTile({ videoTrack }: Props) {
  ...
  return (
    ...
    >
      {/* Live HUD video from Python VideoRenderer — shown when moderator is in the room */}
      {videoTrack && (
        <VideoTrack
          trackRef={videoTrack}
          style={{
      ...
      {/* Fallback content when no video (moderator not yet in room) */}
      {!videoTrack && (
        <>
          <div
            style={{
          ...
          <div style={{ width: "100%", maxWidth: 220 }}>
            ...
            {sorted.length === 0 ? (
              <p style={{ color: "#374151", fontSize: 13, textAlign: "center" }}>
                {state.status === "lobby" ? "Waiting for game to start…" : "No scores yet"}
              </p>
            ) : (
              sorted.map(([identity, score], i) => (
                ...
                >
                  ...
                  <span style={{ fontSize: 14, fontWeight: 700, color: i === 0 ? "#fbbf24" : "#f0f0f0", flexShrink: 0, marginLeft: 8 }}>
                    {score}
                  </span>
                </div>
              ))
      ...
      {/* Winner overlay — shown on top of video or fallback when game ends */}
      {state.winner && (
        <div
          style={{
            position: "absolute",
        ...
        >
          ...
          <div style={{ fontSize: 18, fontWeight: 700, color: "#fbbf24", marginBottom: 4, textAlign: "center" }}>
            {state.winner.displayName ?? state.winner.identity}
          </div>
          <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 14 }}>wins the game!</div>
