...
class SessionStore {
  ...
  async createSession(req: CreateSessionRequest): Promise<SessionState> {
    ...
    const session: SessionState = {
      ...
      floor: { mode: "open" },
      round: null,
      trivia: req.game === "trivia" ? { topic: (req.metadata?.topic as string) ?? "General Knowledge", difficulty: undefined, question_count: 10, question_index: 0 } : null,
      quickdraw: req.game === "quick_draw" ? { category: (req.metadata?.category as string) ?? null, difficulty: undefined, prompt_count: 10, prompt_index: 0, round_duration_ms: 25000, current_prompt: "Draw something" } : null,
